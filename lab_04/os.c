...
//*FILL THIS IN****
  ...
  int32_t Sleep;  		// nonzero if this thread is sleeping
	int32_t Priority;   // priority of the thread control block
};
...
// **similar to Lab 3. initialize priority field****
  ...
  tcbs[2].next = &tcbs[3]; // 2 points to 3
	...
	//setting blocked and sleeping states
	...
	//setting the stack for each thread
...
// Same as Lab 3
	...
	DisableInterrupts();
  *semaPt = *semaPt + 1;
	if(*semaPt <=0){ //if unlocked wake up a new thread
		pt = RunPt->next;
...
// ****IMPLEMENT THIS****
// Same as Lab 3
	PutI = GetI = 0;  //start at zero positon 
	OS_InitSemaphore(&CurrentSize, 0);
	LostData = 0; //no lost data
}
...
//     PD6 is falling edge event
	GPIO_PORTD_IS_R &= ~0x40;   // (d) PD6 are edge-sensitive 
  GPIO_PORTD_IBE_R &= ~0x40;  // PD6 are not both edges 
  GPIO_PORTD_IEV_R &= ~0x40;  // PD6 falling edge event 

// (e) clear PD6 flag
  GPIO_PORTD_ICR_R = 0x40;    // (e) clear flags
// (f) arm interrupt on PD6
	GPIO_PORTD_IM_R |= 0x40;    // (f) arm interrupt on PD6
// priority on Port D edge trigger is NVIC_PRI0_R	31 – 29
	NVIC_PRI0_R = (NVIC_PRI0_R&0x0FFFFFFF)|(priority << 29); 
// enable is bit 3 in NVIC_EN0_R
	NVIC_EN0_R = 0x000000008;
}
...
//***IMPLEMENT THIS***
	// step 1 acknowledge by clearing flag6
	GPIO_PORTD_ICR_R =0x40; // acknowledge flag and clear RIS flag 
